...
class Scanner:
    def __init__(self, fileName):
        ...
        self.pifFileName = "output/" + path.splitext(path.basename(fileName))[0] + "_pif.txt"
        # clear file if already exist
        open(self.pifFileName, 'w').close()
        # file to store the identifiers table
        self.outputIdentifiersTable = "output/" + path.splitext(path.basename(fileName))[0] + "_id_table.txt"
        # file to store the constants table
        self.outputConstantsTable = "output/" + path.splitext(path.basename(fileName))[0] + "_const_table.txt"
